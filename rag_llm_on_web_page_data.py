...
def retriever_similarity(vectorstore, query: str) -> List[Document]:
    ...
    docs, scores = zip(*results_sorted)
...
def create_vectorstore_from_doc_splits(splits, model_name):
...
#docs_similarity_sorted = sorted(docs_sim, key=lambda x: x.metadata["score"], reverse=True)
print(docs_sim)
